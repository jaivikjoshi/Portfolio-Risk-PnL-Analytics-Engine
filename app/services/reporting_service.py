...
from app.db.repositories import get_portfolio, list_instruments, list_prices, list_trades
...
def get_positions(db: Session, *, portfolio_id: str, as_of: date) -> list[dict[str, Any]]:
    trades, prices, instruments = _load_inputs(db, portfolio_id, as_of)
...
def get_pnl(
    ...
    trades, prices, instruments = _load_inputs(db, portfolio_id, end_date)
    return compute_pnl(
        trades,
        prices,
        instruments,
